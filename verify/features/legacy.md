...
Verify: `python3 scripts/verify.py verify legacy` tests authentication, PTY reattach/takeover, ACP prompt/wait/attention, approvals, subtree kill, offline output, and relay restart with tmux-hosted agents (a private tmux server). `scripts/check_legacy.py --source PATH` independently tests the immutable upstream baseline.
