...
Metadata and retained history survive restart, and so do PTY processes: each block runs under a small holder process in its own session that owns the PTY and reaps the child, so a daemon that is killed leaves the block running. On restart the daemon re-adopts every held block (same block ID, same PID) and journals an `adopted` event that says how many bytes the holder buffered while no daemon was attached and how many it had to drop (256 KiB ring per block); a child that exited meanwhile is journaled as `exited` with its code. A block whose holder is gone stays `interrupted`. History from an unclean daemon epoch is conservatively marked incomplete, including adopted and exited blocks; the event API returns an `indeterminate` envelope with the retained events. Structured (ACP) sessions do not survive a restart yet. Retention-expired cursors return `history_gap` and the available range. Fresh work after recovery has its own complete/incomplete record. A storage writer failure refuses further durable mutations. A fault in one ACP reader marks that block’s history incomplete without disabling unrelated controls.
